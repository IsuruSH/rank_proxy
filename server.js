...
app.get('/calculateRank', async (req, res) => {
    ...
    if (!studentData) {
      ...
      try {
        studentData = await Promise.allSettled(
          Array.from({ length: endNumInt - startNumInt + 1 }, (_, i) => fetchStudentData(startNumInt + i))
        );
  
        studentData = studentData
          .filter(result => result.status === 'fulfilled' && result.value.data[gpatype] !== 'NaN')
          .map(result => ({
    ...
    try {
      ...
      const rank = studentData.findIndex(student => student.stnum === stNumInt) + 1;
      const highestGpa = studentData[0].gpa;
      const lowestGpa = studentData[studentData.length - 1].gpa;
      const averageGpa = studentData.reduce((acc, curr) => acc + curr.gpa, 0) / totalCount;
